...
public class Sorting2DAssign {
			...
			try {
				...
				if (num >= 1 && num <= 9) {
					...
					case 6:
						...
						break;
					case 7:
						nameSelectSort(studentInfo, true);
						timeSent = (int) System.currentTimeMillis();
						break;
					case 8:
						nameSelectSort(studentInfo, false);
						timeSent = (int) System.currentTimeMillis();
						break;
					case 9:
						markSelectSort(studentInfo);
						timeSent = (int) System.currentTimeMillis();
						break;
					}// end of switch
				} else {
	...
	public static void nameSelectSort(String[][] a, boolean fn) {
		int i, k, min;
		for (i = 0; i < a.length; i++) {
			min = i;
			for (k = i + 1; k < a.length; k++) {
				if (fn == true) {
					if (a[min][1].compareTo(a[k][1]) > 0) {
						min = k;
					} // end of if
				} else {
					if (a[min][2].compareTo(a[k][2]) > 0) {
						min = k;
					} // end of if
				} // end of else
				if (min != i) {
	...
	}// end of nameSelectSort

	public static void markSelectSort(String[][] a) {
		int i, k, min;
		for (i = 0; i < a.length; i++) {
			min = i;
			for (k = i + 1; k < a.length; k++) {
				if (Float.valueOf(a[k][3]) < Float.valueOf(a[min][3])) {
					min = k;
				} // end of if
			} // end of for
			if (min != i) {
